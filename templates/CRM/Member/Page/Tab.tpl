    ...
    {if $action ne 1 and $action ne 2 and $permission EQ 'edit'}
        <div id="help">
            {if $permission EQ 'edit'}
              ...
              {if $newCredit}
                    ...
                    {ts 1=$newCreditURL}Click <a href='%1'>Submit Membership</a> to process a Membership on behalf of the member using their credit card.{/ts}
                {/if}
            {else}
        ...
        <div class="action-link">
            ...
            {if $accessContribution and $newCredit}
                <a accesskey="N" href="{$newCreditURL}" class="button"><span><div class="icon add-icon"></div>{ts}Submit Membership{/ts}</span></a><br /><br />
            {else}
                <br/ ><br/ >
            {/if}
        </div>
