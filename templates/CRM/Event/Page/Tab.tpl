...
{else}
    ...
    <div id="help">
        ...
        {if $accessContribution and $newCredit}
            ...
            {ts 1=$newCreditURL}Click <a href='%1'>Submit Event Registration</a> to process a new New Registration on behalf of the participant using their credit card.{/ts}
        {/if}
        </p>
    </div>
    {if $action eq 16 and $permission EQ 'edit'}
       <div class="action-link">
           <a accesskey="N" href="{$newEventURL}" class="button"><span><div class="icon add-icon"></div>{ts}Add Event Registration{/ts}</span></a>
            {if $accessContribution and $newCredit}
                <a accesskey="N" href="{$newCreditURL}" class="button"><span><div class="icon add-icon"></div>{ts}Submit Event Registration{/ts}</a></span>
            {/if}
            <br/ ><br/ >
       </div>
   {/if}
