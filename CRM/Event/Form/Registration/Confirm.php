...
class CRM_Event_Form_Registration_Confirm extends CRM_Event_Form_Registration {
  ...
  static function processContribution(&$form, $params, $result, $contactID,
    ...
    $paymentProcessor=CRM_Utils_Array::value('payment_instrument_id', $params);
    if (!CRM_Utils_Array::value('is_pay_later', $params)) {
       $contribParams['payment_instrument_id']=$params['payment_instrument_id'];
    }
   // print_r($params);echo "after is";print_r($contribParams);exit;
    if (!$pending && $result) {
      $contribParams += array(
